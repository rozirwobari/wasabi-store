...
@section('scripts')
    <script>
        ...
        function handleImageFiles(files) {
            // var gambars = []; // Array untuk menampung URL gambar
            Array.from(files).forEach(file => {
                if (file.type.startsWith('image/')) {
                    if (file.size <= 5 * 1024 * 1024) { // 5MB limit
                        if (imagePreviewContainer.children.length < 10) { // Max 10 images
                            const reader = new FileReader();
                            reader.onload = (e) => {
                                // gambars.push(e.target.result); 
                                // document.getElementById('gambars').value = JSON.stringify(gambars);
                        ...
                        alert(`File ${file.name} terlalu besar. Maksimal 5MB`, 'warning');
                    }
                } else {
                    alert(`File ${file.name} bukan format gambar yang valid`, 'warning');
                }
            });
        }
        ...
        function createImagePreview(src, fileName) {
            ...
            previewDiv.className = 'image-preview d-inline-block position-relative';
